...
type Wrapper struct {
	...
	Message   protoreflect.ProtoMessage `bson:"message"`
}
...
func (c *Connection) Marshal(msg protoreflect.ProtoMessage, opts ...MarshalOption) ([]byte, error) {
	...
	protoField := c.pbfld[reflect.TypeOf(msg)]
	fvw, err := dw.WriteDocumentElement(protoField)
	if err != nil {
		return []byte{}, err
	}
	pc := &ProtoCodec{}
...
func (c *Connection) Unmarshal(bson []byte, msg proto.Message) error {
...
func (pc *ProtoCodec) EncodeValue(ctx bsoncodec.EncodeContext, vw bsonrw.ValueWriter, val reflect.Value) error {
	pb := val.Interface().(protoreflect.ProtoMessage)
...
func (pc *ProtoCodec) DecodeValue(ctx bsoncodec.DecodeContext, vr bsonrw.ValueReader, val reflect.Value) error {
	...
	// create pointer to proto struct
	...
	for _, fvr, err := dr.ReadElement(); err != bsonrw.ErrEOD; _, fvr, err = dr.ReadElement() {
		if err != nil {
			return err
		}
		if fvr.Type() == bsontype.EmbeddedDocument {
			// proto is the only top-level embedded document
			pb := ptr.Interface().(protoreflect.ProtoMessage)
			if err := pc.readMessage(pb.ProtoReflect(), fvr); err != nil {
				return err
			}
		} else {
			// no need to read other fields
			if err := vr.Skip(); err != nil {
				return err
			}
		}
...
func MarshalProto(msg protoreflect.ProtoMessage) ([]byte, error) {
	...
	pc := &ProtoCodec{}
	if err := pc.writeMessage(msg.ProtoReflect(), vw); err != nil {
		return []byte{}, err
	}
