...
resource "aws_route53_record" "cname" {
  for_each = var.hosts
  name     = "go-${each.key}${local.host_ext}.anqaml.com"
  records  = ["${azurerm_cdn_endpoint.go_app[each.key].name}.azureedge.net"]
...
resource "null_resource" "custom_domain" {
  for_each   = var.hosts
  depends_on = [azurerm_cdn_endpoint.go_app]
  ...
  provisioner "local-exec" {
    command = <<-EOT
      curl -X PUT \
        ...
        ${local.profile_root}/${azurerm_cdn_profile.go_app.name}/endpoints/${azurerm_cdn_endpoint.go_app[each.key].name}/customDomains/${each.key}${var.env}/?api-version=2019-12-31
    EOT
  }
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      curl -X POST \
        ...
        ${local.profile_root}/${azurerm_cdn_profile.go_app.name}/endpoints/${azurerm_cdn_endpoint.go_app[each.key].name}/customDomains/${each.key}${var.env}/enableCustomHttps?api-version=2019-12-31
    EOT
  }
  ...
  provisioner "local-exec" {
    command = <<-EOT
      curl -X PUT \
        ...
        ${local.profile_root}/${azurerm_cdn_profile.go_app.name}/endpoints/${azurerm_cdn_endpoint.go_app[each.key].name}/customDomains/${each.value}${var.env}/?api-version=2019-12-31
    EOT
  }
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      curl -X POST \
        ...
        -H "Content-Type: application/json" \
        -d '{ "certificateSource":"Cdn", "certificateSourceParameters": { "@odata.type":"Microsoft.Azure.Cdn.Models.CdnCertificateSourceParameters", "certificateType":"Dedicated" }, "protocolType":"ServerNameIndication" }' \
        ${local.profile_root}/${azurerm_cdn_profile.go_app.name}/endpoints/${azurerm_cdn_endpoint.go_app[each.key].name}/customDomains/${each.value}${var.env}/enableCustomHttps?api-version=2019-12-31
    EOT
  }
