...
func New(user, pwd, host string) (Store, error) {
	...
	cnn, err := pbson.Connect(
		...
		pbson.Host(host),
		pbson.Configure(cfg),
		// pbson.Proto(&pbk.Person{}),
		pbson.Audit("kyccertifier"), // cfg sets: audit?
	)
	if err != nil {
	...
	return Store{
		cnn:  cnn,
		coll: cnn.Database.Collection("kyccertifier"),
	}, nil
}

// // Add creates a new kyccertifier record
// func (s *Store) Add(tenant, user string, p *pbk.Person) (*pbk.Person, error) {
...
// // Get returns the kyccertifier specified by id.
...
// // Update applies an update to the specified kyccertifier record.
...
// // Delete deletes an kyccertifier record
